...
class AuthorFragment(private val mViewModel: LibraryViewModel) : Fragment(), View.OnClickListener {
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        mViewModel.getSelectedAuthor().observe(viewLifecycleOwner) { author: Author? ->
            this.author = author
    ...
    private fun populateBooks() {
        mViewModel.selectedLibrary.observe(viewLifecycleOwner) { library ->
            booksTable!!.removeAllViews()
            val books: List<Book> = author?.let { library.getBooksForAuthor(it) } ?: listOf()
